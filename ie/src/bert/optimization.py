...
#          if 'token_label_output_layer' not in tvar.name:
      update_var_list.append(tvar)
 # train_op = tf.train.AdamOptimizer(learning_rate).minimize(loss, global_step=global_step, var_list=update_var_list)
  grads = tf.gradients(loss, update_var_list)
  # This is how the model was pre-trained.
  (grads, _) = tf.clip_by_global_norm(grads, clip_norm=1.0)

  train_op = optimizer.apply_gradients(
      zip(grads, update_var_list), global_step=global_step)
