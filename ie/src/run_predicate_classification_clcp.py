...
class InputFeatures(object):
    ...
    def __init__(self,
                 ...
                 fit_labelspace_mask_positions,
                 fit_labelspace_given_positions,
                 mask_lm_ids,
        ...
        self.fit_labelspace_mask_positions =fit_labelspace_mask_positions
        self.fit_labelspace_given_positions =fit_labelspace_given_positions
...
def convert_single_example(ex_index, example, token_label_list, label_list, max_seq_length,
    ...
    if isinstance(example, PaddingInputExample):
        return InputFeatures(
            ...
            fit_labelspace_mask_positions=[0] * len(label_list),
            fit_labelspace_given_positions=[0] * len(label_list),
            mask_lm_ids=[0] * len(label_list)
        )
    ...
    if ex_index < 3:
        ...
        tf.logging.info("fit_labelspace_mask_positions: %s" % " ".join([str(x) for x in fit_labelspace_mask_positions]))
        tf.logging.info("fit_labelspace_given_positions: %s" % " ".join([str(x) for x in fit_labelspace_given_positions]))
        tf.logging.info("mask_lm_ids: %s" % " ".join([str(x) for x in mask_lm_ids]))
    feature = InputFeatures(
        ...
        is_real_example=True,
        fit_labelspace_mask_positions=fit_labelspace_mask_positions,
        fit_labelspace_given_positions=fit_labelspace_given_positions,
...
def gather_indexes(sequence_tensor, positions, expected_rank=3):
  """Gathers the vectors at the specific positions over a minibatch."""
  sequence_shape = modeling.get_shape_list(sequence_tensor, expected_rank)
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, token_label_list, label_list,
        ...
        features["fit_labelspace_mask_positions"]=create_int_feature(feature.fit_labelspace_mask_positions)
        features["fit_labelspace_given_positions"]=create_int_feature(feature.fit_labelspace_given_positions)
        features["mask_lm_ids"] = create_int_feature(feature.mask_lm_ids)
...
def file_based_input_fn_builder(input_file, seq_length, label_length,
    ...
    name_to_features = {
        ...
        "fit_labelspace_mask_positions" : tf.FixedLenFeature([label_length-1], tf.int64),
        "fit_labelspace_given_positions" : tf.FixedLenFeature([1], tf.int64),
        "mask_lm_ids" :tf.FixedLenFeature([label_length-1], tf.int64),
    }
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 token_label_ids, labels, positions, doc_positions, num_token_labels, num_labels, use_one_hot_embeddings,fit_labelspace_mask_positions,fit_labelspace_given_positions,mask_lm_ids):
    ...
    def joint_embedding(doc_output_layer, token_label_output_layer):
    ...
    H_enc = joint_embedding(doc_output_layer, token_label_output_layer)
    with tf.variable_scope("token_label_loss"):
        if is_training:
            H_enc = tf.nn.dropout(H_enc, keep_prob=0.9)
        input_tensor_mask = gather_indexes(token_label_output_layer, fit_labelspace_mask_positions)
        given_tensor = gather_indexes(token_label_output_layer, fit_labelspace_given_positions)
   
        input_tensor_mask = tf.layers.dense(
            input_tensor_mask,
        ...
        input_tensor_mask = modeling.layer_norm(input_tensor_mask)
        given_tensor = tf.layers.dense(
            given_tensor,
            units=bert_config.hidden_size,
            activation=modeling.get_activation(bert_config.hidden_act),
            kernel_initializer=modeling.create_initializer(
                bert_config.initializer_range))
        given_tensor = modeling.layer_norm(given_tensor)
        ...
        output_weights_mask = tf.get_variable(
            "output_weights_mask",
            shape=[2, bert_config.hidden_size*2],
            initializer=modeling.create_initializer(bert_config.initializer_range))
        fit_labelspace_given_positions_shape=tf.shape(fit_labelspace_given_positions)        
        
        given_tensor=tf.reshape(given_tensor,[fit_labelspace_given_positions_shape[0],FLAGS.mask_num,-1])#bs,mask_num,768        
        given_tensor  = tf.broadcast_to(given_tensor, [fit_labelspace_given_positions_shape[0], num_labels-FLAGS.mask_num, bert_config.hidden_size]) #32,54-unmask,768
        given_tensor =tf.reshape(given_tensor,[-1,bert_config.hidden_size])        
        concat_tensor = tf.concat([input_tensor_mask,given_tensor],axis=1)
        
        logits_mask = tf.matmul(concat_tensor, output_weights_mask, transpose_b=True)
        logits_mask = tf.nn.bias_add(logits_mask, output_bias_mask)
        mask_log_probs = tf.nn.log_softmax(logits_mask, axis=-1)#bs,54-unmask,2
 
        label_ids = tf.cast(labels, tf.float32)
        mask_label_ids =tf.reshape(mask_lm_ids,[-1]) 
        mask_label_ids =tf.one_hot(mask_label_ids , depth=2, dtype=tf.float32)
        per_example_loss_mask = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_mask, labels=mask_label_ids),axis=-1)
    ...
    return loss+5*loss_mask, per_example_loss, probabilities, predict_ids, labels

def get_masked_lm_output(bert_config, input_tensor, output_weights,fit_labelspace_mask_positions,
                         label_ids):
  """Get loss and log probs for the masked LM."""
  input_tensor = gather_indexes(input_tensor, fit_labelspace_mask_positions)
...
def model_fn_builder(bert_config, num_token_labels, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        fit_labelspace_mask_positions = features["fit_labelspace_mask_positions"]
        fit_labelspace_given_positions = features["fit_labelspace_given_positions"]
        mask_lm_ids =features["mask_lm_ids"]
        ...
        (total_loss, token_label_per_example_loss, token_label_logits, token_label_predictions, token_label_ids_labelspace) = create_model(
            ...
            num_token_labels, num_labels, use_one_hot_embeddings,fit_labelspace_mask_positions,fit_labelspace_given_positions,mask_lm_ids)
...
def run_pred():
    ...
    processors = {
        "laco_clcp": Multi_Label_Classification_Processor,
    }
...
def predicate_classification_model_train(job_id, schema_set, train_data_path, train_para):
    ...
    path = os.path.join(ie_save_path_re, "model/predicate_infer_out_clcp/")
...
def predicate_classification_model_test(job_id, schema_set, train_data_path, clf_model_idx):
    ...
    path = os.path.join(pretrain_model_path, "cased_L-12_H-768_A-12/bert_config.json")
    FLAGS.bert_config_file = path
    path = os.path.join(ie_save_path_re, "model/predicate_infer_out_clcp/"+clf_model_idx)
